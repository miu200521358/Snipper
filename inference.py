...
def get_args_parser():
    ...
    parser.add_argument(
        "--resume",
        default="C:/Users/shihaozou/Desktop/models/12-06_20-17-34/checkpoint.pth",
...
def main(args):
    ...
    with torch.set_grad_enabled(False):  # deactivate autograd to reduce memory usage
        for samples in tqdm(all_samples):
            ...
            input_size = (
                samples["input_size"].to(device).unsqueeze(0).unsqueeze(0).unsqueeze(0)
            )  # [1, 1, 1, 2]
    ...
    save_dir = os.path.join(
        ...
        f"{os.path.basename(args.data_dir)}_predictions",
    )
    ...
    if args.vis_heatmap_frame_name is None:
        ...
        if not os.path.exists(
            "{}/{}_predictions/track2d/{}_track.jpg".format(
                args.output_dir, seq_name, frame_name
            )
        ):
            raise ValueError(
                "frame {} is not used for prediction".format(
                    args.vis_heatmap_frame_name
                )
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "Snipper inference script", parents=[get_args_parser()]
    )
