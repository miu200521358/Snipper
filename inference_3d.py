...
from inference_utils import (associate_snippets, get_all_samples,
                             save_results_3d, save_visual_results_2d)
...
def get_args_parser():
    ...
    parser.add_argument('--target_dir', default='C:/Users/shihaozou/Desktop/exps',
...
def main(args):
    ...
    save_visual_results_2d(all_frames_results, all_filenames, args.data_dir, args.output_dir, max_pid, args.max_depth, args.seq_gap)

    print('save visual results of each frame in {}'.format(args.output_dir))
    save_results_3d(all_frames_results, all_filenames, args.data_dir, args.output_dir, max_pid, args.max_depth, args.seq_gap)


if __name__ == '__main__':
    parser = argparse.ArgumentParser('Snipper inference script', parents=[get_args_parser()])
    args = parser.parse_args()
    args.data_dir = os.path.join(args.target_dir, "frames")
    frames = glob(f"{args.data_dir}/*.*")
    img = Image.open(frames[0])
    args.output_dir = os.path.join(args.target_dir, "results")
